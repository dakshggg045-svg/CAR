...
export default function AdminVehicleForm({ mode }: { mode: 'add' | 'edit' }) {
  ...
  const [rate, setRate] = useState(50); const [cat, setCat] = useState<VehicleCategory>('Sedan');
  const [status, setStatus] = useState<VehicleStatus>('available'); const [color, setColor] = useState('');
  const [plate, setPlate] = useState(''); const [miles, setMiles] = useState(0);
  ...
  const submit = () => {
    ...
    dispatch({ type: mode === 'edit' ? 'UPDATE_VEHICLE' : 'ADD_VEHICLE', payload: v });
    dispatch({ type: 'SET_NOTIFICATION', payload: { type: 'success', message: `${make} ${model} ${mode === 'edit' ? 'updated' : 'added'}` } });
    navigate('admin-vehicles');
  };

  const f = "w-full px-3 py-2 bg-[#1a1a1a] border border-white/[0.08] rounded-lg text-white text-sm focus:border-amber-500/50 focus:outline-none";
  const lb = "block text-xs text-slate-500 mb-1";

  return (
    <div className="min-h-screen bg-[#0c1222] pt-[60px]">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="flex items-center gap-3 mb-5">
          <button onClick={() => navigate('admin-vehicles')} className="text-slate-500 hover:text-white transition-colors"><ArrowLeft className="w-4 h-4" /></button>
          <h1 className="text-lg font-bold text-white">{mode === 'edit' ? 'Edit vehicle' : 'Add vehicle'}</h1>
        </div>
        ...
        <div className="space-y-5">
          ...
          <div className="border border-slate-800 rounded-lg p-4 bg-slate-900">
            <h2 className="text-sm font-medium text-white mb-3">Pricing</h2>
          ...
          <div className="border border-slate-800 rounded-lg p-4 bg-slate-900">
            <h2 className="text-sm font-medium text-white mb-3">Specs</h2>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <div className="flex items-end gap-4 pb-1">
                <label className="flex items-center gap-1.5 text-sm text-slate-400 cursor-pointer"><input type="checkbox" checked={ac} onChange={e => setAc(e.target.checked)} className="accent-amber-500" />A/C</label>
                <label className="flex items-center gap-1.5 text-sm text-slate-400 cursor-pointer"><input type="checkbox" checked={gps} onChange={e => setGps(e.target.checked)} className="accent-amber-500" />GPS</label>
              </div>
            </div>
          </div>
          ...
          <div className="border border-slate-800 rounded-lg p-4 bg-slate-900">
            <div className="flex items-center justify-between mb-3">
              <h2 className="text-sm font-medium text-white">Images</h2>
              <button onClick={() => setImgs(VEHICLE_IMAGES[cat.toLowerCase()] || VEHICLE_IMAGES['sedan'])} className="text-xs text-amber-400">Use defaults</button>
            </div>
            {imgs.length > 0 && (
              <div className="flex flex-wrap gap-2 mb-3">
                {imgs.map((u, i) => (
                  <div key={i} className="relative w-20 h-14 rounded overflow-hidden bg-slate-800 group">
                    <img src={u} alt="" className="w-full h-full object-cover" />
                    <button onClick={() => setImgs(imgs.filter((_, j) => j !== i))} className="absolute top-0.5 right-0.5 w-4 h-4 bg-red-600 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><X className="w-2.5 h-2.5 text-white" /></button>
                  </div>
                ))}
              </div>
            )}
            <div className="flex gap-2">
              <input type="url" value={newUrl} onChange={e => setNewUrl(e.target.value)} placeholder="https://…" className={`flex-1 ${f}`} onKeyDown={e => { if (e.key === 'Enter' && newUrl.trim()) { setImgs([...imgs, newUrl.trim()]); setNewUrl(''); } }} />
              <button onClick={() => { if (newUrl.trim()) { setImgs([...imgs, newUrl.trim()]); setNewUrl(''); } }} className="px-3 py-2 bg-slate-800 text-slate-400 rounded text-sm hover:text-white"><Plus className="w-3.5 h-3.5" /></button>
            </div>
          </div>

          <div className="flex gap-2">
            <button onClick={() => navigate('admin-vehicles')} className="px-4 py-2.5 bg-slate-800 text-slate-300 text-sm rounded">Cancel</button>
            <button onClick={submit} className="flex-1 py-2.5 bg-amber-500 hover:bg-amber-400 text-black text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-1.5"><Save className="w-4 h-4" />{mode === 'edit' ? 'Update' : 'Add vehicle'}</button>
          </div>
        </div>
      </div>
    </div>
